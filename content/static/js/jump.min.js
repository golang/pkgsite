...
function updateJumpList(b){lastFilterValue=b;jumpListItems||(jumpListItems=collectJumpListItems());for(setActiveJumpItem(-1);jumpList.firstChild;)jumpList.firstChild.remove();var c=function(u,v,a){return u.name.substr(0,v)+"<b>"+u.name.substr(v,a)+"</b>"+u.name.substr(a)};console.log("filter is",b);if(b){for(var d=b.toLowerCase(),f=[],h=[],n=[],g=$jscomp.makeIterator(jumpListItems),l=g.next();!l.done;l=g.next()){l=l.value;var r=l.name.toLowerCase();r===d?(l.link.innerHTML=c(l,0,l.name.length),f.push(l)):
r.startsWith(d)?(l.link.innerHTML=c(l,0,b.length),h.push(l)):(r=r.indexOf(d),-1<r&&(l.link.innerHTML=c(l,r,r+b.length),n.push(l)))}console.log("exact",f);console.log("prefix",h);console.log("infix",n);b=$jscomp.makeIterator(f.concat(h).concat(n));for(l=b.next();!l.done;l=b.next())jumpList.appendChild(l.value.link)}else for(b=$jscomp.makeIterator(jumpListItems),l=b.next();!l.done;l=b.next())c=l.value,c.link.innerHTML=c.name+" <i>"+c.kind+"</i>",jumpList.appendChild(c.link);jumpBody.scrollTop=0;0<jumpList.children.length&&
setActiveJumpItem(0)}function setActiveJumpItem(b){var c=jumpList.children;0<=activeJumpItem&&c[activeJumpItem].classList.remove("JumpDialog-active");b>=c.length&&(b=c.length-1);if(0<=b){c[b].classList.add("JumpDialog-active");var d=c[b].offsetTop-c[0].offsetTop;c=d+c[b].clientHeight;d<jumpBody.scrollTop?jumpBody.scrollTop=d:c>jumpBody.scrollTop+jumpBody.clientHeight&&(jumpBody.scrollTop=c-jumpBody.clientHeight)}activeJumpItem=b}
