...
function collectJumpListItemsFallback(b){var c=[],d={};b=$jscomp.makeIterator(b.querySelectorAll("*[id]"));for(var f=b.next();!f.done;f=b.next()){f=f.value;var h=f.getAttribute("id");!d[h]&&/^[^_][^-]*$/.test(h)&&(d[h]=!0,c.push(newJumpListItem(f)))}return c}function newJumpListItem(b){var c=document.createElement("a"),d=b.getAttribute("id");c.setAttribute("href","#"+d);c.setAttribute("tabindex","-1");b=b.getAttribute("data-kind");return{link:c,name:d,kind:b,lower:d.toLowerCase()}}
var lastFilterValue,activeJumpItem=-1;
