...
-- having a user defined type for search results simplifies the popular_search
-- function below.
...
-- popular_search is implemented as a stored function, so that we can use a
-- cursor to scan search documents procedurally, and stop scanning early,
-- whenever our search results are provably correct.
CREATE OR REPLACE FUNCTION popular_search(rawquery TEXT, lim INT, off INT)
			...
			FOR i IN 1..last_idx LOOP
				...
				IF top[i] IS NULL OR
					...
					(res.score = top[i].score AND res.commit_time = top[i].commit_time AND
					 res.package_path COLLATE "C" < top[i].package_path COLLATE "C") THEN
