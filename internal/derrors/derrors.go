...
func (t ErrorType) String() string {
	return [...]string{"nil", "Not Found", "Not Acceptable", "Invalid Argument", "Uncategorized"}[t]
}
...
func (e dError) Error() string {
	return fmt.Sprintf("%s: %s", e.typ.String(), e.message)
}
...
func StatusError(code int, format string, args ...interface{}) error {
	...
	if code == http.StatusNotAcceptable {
		return NotAcceptable(format, args...)
	}
...
// NotAcceptable creates a new error that indicates the requested entity is not
// acceptable.
func NotAcceptable(format string, args ...interface{}) error {
	return dError{
		typ:     NotAcceptableType,
		message: fmt.Sprintf(format, args...),
	}
}

// IsNotAcceptable reports whether err is a NotAcceptable error.
func IsNotAcceptable(err error) bool {
	de, ok := err.(dError)
	return ok && de.typ == NotAcceptableType
}
