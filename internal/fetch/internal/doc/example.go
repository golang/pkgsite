...
type Example struct {
	Name        string // name of the item being exemplified (with optional suffix)
...
// classifyExamples moves relevant examples in pkg.Examples into the appropriate
// Examples field in Type or Func that the example is associated with.
//
// Note that classification is actually ambiguous in some cases:
//	* ExampleFoo_Bar matches a type named Foo_Bar or a method named Foo.Bar.
//	* ExampleFoo_bar matches a type named Foo_bar or Foo (with a bar suffix).
//
func classifyExamples(pkg *Package) {
	allExamples := pkg.Examples
	if len(allExamples) == 0 {
		return
	}
	pkg.Examples = pkg.Examples[:0]

	// Mapping of names for funcs, types, and methods to the example listing.
	ids := make(map[string]*[]*Example)
	ids[""] = &pkg.Examples // package-level examples have an empty name
	...
	for _, t := range pkg.Types {
		if ast.IsExported(t.Name) {
			...
			for _, m := range t.Methods {
				if ast.IsExported(m.Name) && m.Level == 0 { // avoid forwarded methods from embedding
	...
	for _, ex := range allExamples {
		for i := len(ex.Name); i >= 0; i = strings.LastIndexByte(ex.Name[:i], '_') {
			prefix, suffix := ex.Name[:i], strings.TrimPrefix(ex.Name[i:], "_")
			if exs, ok := ids[prefix]; ok && !ast.IsExported(suffix) && ex.Name[i:] != "_" {
				ex.Suffix = strings.Replace(strings.Title(suffix), "_", " ", -1)
				*exs = append(*exs, ex)
				break
			}
		}
		// TODO: What should we do with malformed example names?
	}
